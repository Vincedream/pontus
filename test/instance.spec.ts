import pontus, { PontusRequestConfig, PontusResponse } from '../src/index'
...
describe('instance', () => {
  ...
  test('should use instance options', () => {
    const instance = pontus.create({ timeout: 1000 })
  ...
  test('should have defaults.headers', () => {
    const instance = pontus.create({ baseURL: 'https://api.example.com' })
  ...
  test('should get the computed uri', () => {
    const fakeConfig: PontusRequestConfig = {
    ...
    expect(pontus.getUri(fakeConfig)).toBe(
